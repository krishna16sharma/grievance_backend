const handleDept = (req,res,db) =>{
    ...
    .catch(state=>{
        ...
        .catch(ut=>{
                db.select('ut_user_id','ut_abbreviation').from('union_territory_user')
                .where('person_id','=',user.person_id)
                .then(
                    d_id =>{
                        console.log("D_User",d_id[0]);
                        db.select('grievance_id').from('ut_handles')
                        .where('ut_abbreviation','=',d_id[0].ut_abbreviation)
                        .then(
                            grievances =>{
                                console.log('Grievances', grievances, grievances[0])
                                db.select('*').from('grievance')
                                .where('grievance_id','ilike','%'+d_id[0].ut_abbreviation+'%')
                                .then(details =>{
                                    console.log('Retrieved!', details, details[0])
                                    res.json(details)
                                })
